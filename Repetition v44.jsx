...
function draw() {
	...
	// this controls the center of gravity
	var centerX = 0;
	var centerY = b.height/2;

	// this controls the detail of the journey words
	var limit = 10; // the number of journey words
	var contents = "London test";
	...
	// this controls the detail of the outside stroke effect
	var steps = 5;
	var maxStrokeWeight = 30;
	var strokeColor1 = b.color(0, 0, 0, 0);
	var strokeColor2 = b.color(0, 100, 100, 0);
	...
	for (var i = 0; i < myArray.length; i++) {
		...
		var posY = stepY * i;
		var randomSize = b.floor(b.random(100, 150));
		...
		// 3: making the destination word look right
		...
		if (b.bounds(myWords[0]).right > b.width) {
			...
			// creating the outside stroke effect on the destination word
			for (var k = steps; k > 0; k++) {
				var outsideStrokeText = b.text(string, posX, posY, b.width, b.height);
				var mappedStrokeWeight = b.map(k, steps, 0, maxStrokeWeight, 0);

				if (i%2 == 0) {
					b.typo(outsideStrokeText, 'strokeColor', strokeColor1);
				} else {
					b.typo(outsideStrokeText, 'strokeColor', strokeColor2);
				}

				b.typo(outsideStrokeText, 'strokeWeight', mappedStrokeWeight);
				b.typo(outsideStrokeText, 'strokeAlignment', TextStrokeAlign.OUTSIDE_ALIGNMENT);
				b.typo(outsideStrokeText, 'endJoin', OutlineJoin.ROUND_END_JOIN);
			}

			// creating the journey words using a for-loop and skewing them all accordingly
			...
			// this is what happens if the destination word doesn't bleed off the page
