...
function draw() {
	...
	for (var i = 0; i < myCharacters.length; i++) {
		var distance = b.dist(b.bounds(myFrame).left + 0.5 * b.bounds(myFrame).width, b.bounds(myFrame).top + 0.5 * b.bounds(myFrame).height, b.bounds(myCharacters[i]).left + 0.5 * b.bounds(myCharacters[i]).width, b.bounds(myCharacters[i]).top + 0.5 * b.bounds(myCharacters[i]).height);
